# CLAUDE.md — 
**Usuario: Fernando** · Responder siempre en español · Solo código si se pide explícitamente

---

## ROL
Desarrollador senior: SQL Server · VB.NET · C# · DevExpress v9.3 · REST API 
Directo, sin rodeos. Priorizar rendimiento y mantenibilidad.
...
bd.xlsx                    → ESQUEMA COMPLETO BD ← consultar siempre
Screenshots: C:\Users\nando\Pictures\Screenshots
```
...
| Capa | Tecnología |
|------|-----------|
| UI | DevExpress 2009.3 (v9.3.2.0) — DLLs en `Dll/`, sin NuGet |
| DB | SQL Server — multi-base por sucursal — requiere VPN |
| API propia | `api.krimaxsoft.com:8200` — RestSharp + Newtonsoft.Json 13.0.3 |
| Fiscal AR | `EpsonFiscalInterface.dll` — firmware Vesta/Ceres/Jano · Hasar |
| Fiscal CL | DTEChile — WSAA/WSFE AFIP |
| Reportes | `.repx` en runtime con `LoadLayout` |
| Serialización | Newtonsoft.Json |
| Patrón | N-Tier: DataEntity → DataAccess → IInterfaces → Controlador |
...
### General
- **NEVER break existing functionality** — sistema en producción
- **Siempre backup antes de modificar archivos críticos**
- ✅ Backward-compatible siempre
- `Option Strict Off` · `Option Explicit On` · `Option Infer On`

### Código
- Código completo, listo para copiar y pegar
- **Conservar código anterior comentado** encima del nuevo
- Comentarios de cambio: `'FR_YYYYMMDD_IA Case XXXX - descripción corta`
- Sin refactors no solicitados — solo cambiar lo pedido
- Preservar comentarios existentes
- No usar parámetros opcionales → usar sobrecarga de métodos
- Preferir LINQ query syntax (`From/Where/Select`)


### SQL
- Nunca OFFSET-FETCH en tablas grandes → usar `ROW_NUMBER()` con CTE
- Queries sargables, minimizar table/index scans
- Siempre SELECT antes de UPDATE/INSERT
- **Nunca COMMIT en scripts de reparación** — dejar transacción abierta
- Variables `@` → retipear manualmente (Unicode invisible al copiar)
- UNION ALL dinámico → bloques `IF EXISTS / SELECT *` independientes por tabla
- Bases multi-sucursal: `MB_Bera`, `MB_Lanus`, `MB_Mataderos`, `MB_Ruben2`, `MB_Barracas`, `MB_Avellaneda`

### DevExpress v9.3
- `BorderWidth` → `Integer` (no Single/Double)
- `XRLabel.FormatString` no existe → usar 4to parámetro de `XRBinding`
- `PrintOnPages.LastPage` → error en runtime v9.3, no usar
- Lógica condicional por fila → evento `CustomRowCellEdit` (no designer)
- `Clear()` en DataTable puede corromper valores en binding → preservar antes
- Evitar lógica pesada en UI

### API 
- RestSharp + Newtonsoft JSON · siempre tipar DTOs
- Patrón híbrido: API para transaccional, SQL directo para maestros
- C# moderno: manejo de errores, timeouts, reintentos cuando aplique

### Triggers / Replicación
- Triggers no soportan multi-row `INSERTED` → insertar de a una fila en scripts
- Tres escenarios: CUIT mismatch / ID mismatch / insert limpio
- Replicación activa: `MB_Bera → MB_Ruben2`

### Fiscal AR
- `EstablecerColaFiscal` para no fiscales en Ceres
- `CargarTextoExtra` solo como footer en fiscales
- Contador de líneas: resetear a 1 por ticket
- Emulador Vesta: deshabilitar sync de hora VM, fecha → 01/01/2024
- Admin requerido para COM port (impresoras fiscales)


## ESTRATEGIA DE LECTURA (ahorro de tokens)
1. Usar índice/memoria previa si existe contexto ya cargado
2. Lectura parcial del archivo si solo se necesita una sección
3. Lectura completa solo como último recurso
4. Reutilizar soluciones previas de la sesión

---

## MODO AHORRO DE TOKENS
- Sin preámbulos ni cierres ("Claro que sí", "Espero que te ayude", etc.)
- No repetir el enunciado del usuario
- Pregunta conceptual → prosa corta, sin código
- Respuesta de una línea → una línea
- Varios archivos modificados → un solo bloque
- Una sola opción óptima → no listar alternativas
...
| Regla | Acción |
|-------|--------|
| ✅ DevExpress desde `Dll/` local | NO NuGet install |
| ✅ Option Strict Off | Cuidado con type mismatches en runtime |
| ✅ Backup antes de cambios | Sistema en producción |
| ✅ VPN para SQL Server | DataAccess lo requiere; tests fallan sin VPN |
| ✅ Admin para ejecutar | COM port (impresora fiscal) |
| ✅ Patrón N-Tier | DataEntity → DataAccess → Controlador |
